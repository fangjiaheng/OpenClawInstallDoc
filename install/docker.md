...
- **否**：你在自己的机器上运行，只想要最快的开发循环。请改用[正常的安装流程](/install/)。

::: warning 沙箱注意事项
智能体沙箱隔离也使用 Docker，但它不需要完整的 Gateway 网关在 Docker 中运行。参阅[沙箱隔离](#智能体沙箱主机-gateway-网关-docker-工具)。
:::
...
::: warning
从仓库根目录运行 `docker compose ...`。如果你启用了 `OPENCLAW_EXTRA_MOUNTS` 或 `OPENCLAW_HOME_VOLUME`，设置脚本会写入 `docker-compose.extra.yml`；在其他地方运行 Compose 时包含它：
...
- 每个智能体一个容器
- **智能体工作区访问**：
  - `workspaceAccess: "none"`（默认）使用 `~/.openclaw/sandboxes`
...
::: tip 多智能体
通过 `agents.list[].sandbox.{docker,browser,prune}.*` 按智能体覆盖 `agents.defaults.sandbox.{docker,browser,prune}.*`（当 `agents.defaults.sandbox.scope` / `agents.list[].sandbox.scope` 是 `"shared"` 时忽略）。
:::
...
::: warning 重要
如果你使用工具允许列表，添加 `browser`（并从拒绝中移除它）否则工具仍然被阻止。清理规则（`agents.defaults.sandbox.prune`）也适用于浏览器容器。
:::
